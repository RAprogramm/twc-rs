...
Главная фича, которой нет у Python-CLI: полноценная облачная консоль в
терминале (`twc-rs dashboard`) — сайдбар во всю высоту (хаб создания,
проекты, сервисы, настройки) и адаптивная сетка карточек справа.

![Демо дашборда twc-rs](docs/demo/dashboard.gif)

| Клавиша | Действие |
|---|---|
| `↑` `↓` / `k` `j` | перемещение по сайдбару, карточкам и строкам деталей |
| `←` `→` / `h` `l` | перемещение по сетке карточек |
| `Enter` | фокус на контент / открыть детали / нажать кнопку |
| `Esc` | шаг назад: детали → контент → сайдбар |
| `y` / `c` | скопировать выделенное значение (буфер через OSC 52) |
| `n` | создать новый ресурс (где поддерживается) |
| `/` | фильтр текущего списка |
| `Ctrl+K` | палитра команд — действия, тема, язык, смена профиля |
| `p` | смена профиля аккаунта |
| `r` | обновить данные |
| `?` | справка |
| `Q` | выход |

- Мгновенный старт: последний снимок данных рисуется сразу, свежие данные
  подтягиваются по каждому эндпоинту независимо — без пустых экранов.
- Интерактивные детали ресурса на уровне веб-панели: кнопки действий
  (бэкап, редеплой, удаление, …) сверху, любое значение копируется, живые
  спарклайны CPU/RAM, подтверждение для необратимых операций.
- Хаб создания с описанием каждого продукта; создание ресурсов и **смена
  профиля аккаунта** прямо из панели.
- Настройки карточками: четыре true-color темы с предпросмотром палитры,
  EN/RU, переключатели раскладки — всё сохраняется в конфиг.
...
![Демо twc-rs apps list](docs/demo/apps-list.gif)
...
twc-rs apps logs my-api --today           # runtime-логи по имени или ID (также --since, --tail)
twc-rs apps list-deploys my-api           # история деплоев, новые сверху
twc-rs apps deploy-logs my-api            # build/deploy-логи последнего деплоя
twc-rs doctor                             # поиск конфликтующих установок в PATH
twc-rs update                             # самообновление через найденный менеджер пакетов
twc-rs dashboard                          # интерактивная TUI-консоль
```
...
[docs/COMPARISON.md](docs/COMPARISON.md), а полный справочник CLI —
автогенерируемый из определений команд при каждом деплое — на
...
![Демо динамического автодополнения twc-rs](docs/demo/completions.gif)

Подключение вместо (или поверх) статического скрипта:
